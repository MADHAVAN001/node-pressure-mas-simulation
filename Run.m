...
X_AR = zeros(num_paths,total_num_vehicles);
...
for total_num_vehicles = 5:step:150
    ...
    % 
%     vehicle_concentration = zeros(num_nodes,num_nodes);
%     for time_step=1:num_nodes
%         for node_number=1:total_num_vehicles
%             for k=1:num_nodes
%                 if(vehicle_positions(time_step,node_number) == k)
%                     vehicle_concentration(time_step,k) = vehicle_concentration(time_step,k)+1;
%                 end
%             end
%         end
%     end
...
%     for time_step=1:num_nodes
%         for node_number=1:num_nodes
%             if( vehicle_concentration(time_step,node_number)>node_capacity_matrix(node_number,1))
%                 num_affected_nodes = num_affected_nodes+1;
%             end
%         end
%     end
    ...
    for k = total_num_vehicles-step+1:total_num_vehicles
        start = start_end(1,k);
        ending = start_end(2,k);
        path_planning_AR = AR_path( A, distance, num_nodes, start, ending, beta_AR, traffic_repulsion);
        %AR_path( A, distance, num_nodes, start, end, beta_AR, traffic_repulsion)
        X_AR(:,k) = calculate_matrix_i(node_incidence_matrix,path_planning_AR,start, num_paths, num_nodes);    
    end
    
    X_AR_summation = sum(X_AR,2);
    
    %Dijkstra traffic severity
    traffic_initial = 0;
    for tmp = 1: num_nodes
        if(traffic_severity_A(tmp,1) > 0)
            traffic_initial = traffic_initial + exp(traffic_severity_A(tmp,1));
        end
    end
    traffic_A((total_num_vehicles-5)/5+1,1) = traffic_initial;
    
    
    %A* with repulsion traffic severity
    traffic_AR = 0;
    traffic_severity_AR = (A_plus_matrix * X_AR_summation) - node_capacity_matrix;
    for tmp = 1:num_nodes
        if(traffic_severity_AR(tmp,1) > 0)
            traffic_AR = traffic_AR + exp(traffic_severity_AR(tmp,1));
        end
    end
    traffic_AR_total((total_num_vehicles-5)/5+1,1) = traffic_AR;
    ...
    traffic_severity = zeros(200,1);
    num_affected_matrix = zeros(200,1);
    
    node_p = zeros(num_nodes,1);
    
    %Random k shortest paths
...
%     
    ...
    X_rksp_summation = sum(X_rksp,2);
    X_ebrksp_summation = sum(X_ebrksp,2);
    
    traffic_rksp = 0;
    traffic_severity_rksp = (A_plus_matrix * X_rksp_summation) - node_capacity_matrix;
    for tmp = 1:num_nodes
        if(traffic_severity_rksp(tmp,1) > 0)
            traffic_rksp = traffic_rksp + exp(traffic_severity_rksp(tmp,1));
        end
    end

    traffic_ebrksp = 0;
    traffic_severity_ebrksp = (A_plus_matrix * X_ebrksp_summation) - node_capacity_matrix;
    for tmp = 1:num_nodes
        if(traffic_severity_ebrksp(tmp,1) > 0)
            traffic_ebrksp = traffic_ebrksp + exp(traffic_severity_ebrksp(tmp,1));
        end
    end
    traffic_rksp_total((total_num_vehicles-5)/5+1,1) = traffic_rksp;
    traffic_ebrksp_total((total_num_vehicles-5)/5+1,1) = traffic_ebrksp;
...
%             for node_number=1:total_num_vehicles
%                 for k=1:num_nodes
%                     if(vehicle_positions_pressure(time_step,node_number) == k)
%                         vehicle_concentration_i(time_step,k) = vehicle_concentration_i(time_step,k)+1;
%                     end
%                 end
%             end
%         end
...
%         for time_step=1:num_nodes
%             for node_number=1:num_nodes
%                 if( vehicle_concentration_i(time_step,node_number)>node_capacity_matrix(node_number,1))    
%                     num_affected_matrix(j,1) = num_affected_matrix(j,1) + 1;
%                 end
%             end
%         end
...
y1 = 4 * log(traffic_rksp_total);
y2 = 4 * log(traffic_AR_total);
y4 = 4 * log(traffic_ebrksp_total);
